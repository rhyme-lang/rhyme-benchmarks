...
def main():
    ...
    # Collect medians in query order
    medians = []
    for query_file in sql_files:
        q_num = query_file.stem  # e.g. "1", "2", ...
        medians.append((q_num, None))  # placeholder; re-read from file below

    # Re-read results file to get medians in order
    medians = []
    with open(results_file, 'r') as f:
        for i, (query_file, line) in enumerate(zip(sql_files, f)):
            parts = line.strip().split()
            q_num = query_file.stem
            if parts and parts[0] != 'ERROR':
                median_val = float(parts[-2])  # second-to-last is median
                medians.append((q_num, median_val))
            else:
                medians.append((q_num, None))

    # Format 1: LaTeX-style table column
    print("\n--- Medians (LaTeX format) ---")
    for i, (q_num, med) in enumerate(medians):
        val = str(round(med)) if med is not None else "ERROR"
        suffix = "\\\\" if i == len(medians) - 1 else "&"
        print(f"{val}{suffix}")

    # Format 2: (Q#, time) pairs
    print("\n--- Medians (tuple format) ---")
    for q_num, med in medians:
        val = str(round(med)) if med is not None else "ERROR"
        print(f"(Q{q_num}, {val})")
