...
def run_query(query_file, measure=False):
...
def benchmark_query(query_file):
    """Run warmup + 5 timed runs, return results"""
    # Warmup run (throw away)
    run_query(query_file, measure=False)
    
    # 5 benchmark runs with server-side timing
    times = []
    for i in range(5):
        print(f"Run {i+1}")
        elapsed = run_query(query_file, measure=True)
        if elapsed is None:
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 benchmark.py <path_to_sql_files>")
        print("Example: python3 benchmark.py ./queries")
        sys.exit(1)
    ...
    with open(results_file, 'w') as f:
        for query_file in sql_files:
            ...
            try:
                times, median, average = benchmark_query(query_file)
