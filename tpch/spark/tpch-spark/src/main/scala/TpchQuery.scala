...
object TpchQuery {
  ...
  def executeQueries(spark: SparkSession, schemaProvider: TpchSchemaProvider, queryNum: Option[Int], queryOutputDir: String): ListBuffer[(String, Float)] = {
    ...
    for (queryNo <- queryFrom to queryTo) {
      ...
      try {
        ...
        val runTimes = new ListBuffer[Float]

        for (run <- 1 to numRuns) {
          val startTime = System.nanoTime()
          val queryOutput = query.execute(spark, schemaProvider)
          // Only output on the first run
          if (run == 1) {
            outputDF(queryOutput, queryOutputDir, query.getName())
          } else {
            queryOutput.collect() // Force evaluation without saving
          }
          val endTime = System.nanoTime()
          val elapsed = (endTime - startTime) / 1000000.0f // to milliseconds
        ...
        // Calculate median (middle value of sorted 5 runs)
        val sortedTimes = runTimes.sorted
        val median = sortedTimes(sortedTimes.length / 2)
        executionTimes += new Tuple2(query.getName(), median)
      }
      catch {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val inputDataDir = sys.env.getOrElse("TPCH_INPUT_DATA_DIR", "file://" + cwd + "/SF10")
    ...
    if (executionTimes.length > 0) {
      ...
      bw.write(f"Query\tTime (milliseconds)\n")
